...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(in_features=latent_size, out_features=self.hidden_dim),
            nn.ReLU(),
            nn.Linear(in_features=self.hidden_dim, out_features=self.hidden_dim),
            nn.ReLU(),
            nn.Linear(in_features=self.hidden_dim, out_features=self.hidden_dim),
            nn.ReLU(),
            nn.Linear(in_features=self.hidden_dim, out_features=input_size),
            nn.Sigmoid(),
...
class CVAE(nn.Module):
    def __init__(self, input_size, num_classes=10, latent_size=15):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(in_features=input_size + num_classes, out_features=self.hidden_dim), # in_features adjusted to include one-hot vector
            nn.ReLU(),
            nn.Linear(in_features=self.hidden_dim, out_features=self.hidden_dim),
            nn.ReLU(),
            nn.Linear(in_features=self.hidden_dim, out_features=self.hidden_dim),
        ...
        ############################################################################################
        self.decoder = nn.Sequential(
            nn.Linear(in_features=latent_size + num_classes, out_features=self.hidden_dim), # adjusted for one-hot vector
            nn.ReLU(),
            nn.Linear(in_features=self.hidden_dim, out_features=self.hidden_dim),
            nn.ReLU(),
            nn.Linear(in_features=self.hidden_dim, out_features=self.hidden_dim),
            ...
            nn.Linear(in_features=self.hidden_dim, out_features=input_size),
            nn.Sigmoid(),
            nn.Unflatten(dim=1, unflattened_size=(1, 28, 28))  # Change this line
        )
        ############################################################################################
        #                                      END OF YOUR CODE                                    #
        ############################################################################################
    ...
    def forward(self, x, c):
        ...
        hidden_features = self.encoder(torch.cat((x.view(x.size(0), -1), c), dim=1))
        mu = self.mu_layer(hidden_features)
        logvar = self.logvar_layer(hidden_features)
        z = reparametrize(mu, logvar)
...
def reparametrize(mu, logvar):
    ...
    z = torch.exp(0.5 * logvar) * epsilon + mu
    ################################################################################################
    #                              END OF YOUR CODE                                                #
    ################################################################################################
    return z
