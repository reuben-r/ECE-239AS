...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    real_loss = bce_loss(logits_real, torch.ones_like(logits_real).view(logits_real.shape[0]).type(dtype))
    fake_loss = bce_loss(logits_fake, torch.zeros_like(logits_fake).view(logits_fake.shape[0]).type(dtype))
...
def generator_loss(logits_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    loss = bce_loss(logits_fake, torch.ones_like(logits_fake).view(logits_fake.shape[0]).type(dtype))
